...
def generate_events_for_day(target_date: date) -> int:
    inserted_attempts = 0
    with get_connection() as conn:
        ...
        with conn.cursor() as cur:
            ...
            for r in rows:
                try:
                    logging.info(f"➡️ Processing schedule id={r.get('id')} for {target_date}, freq={r.get('frequency_label')}")
                    times = _times_for_schedule_today(r.get("frequency_label"), r, target_date)
                    for tm in times:
                    ...
                    conn.commit()
                    logging.info(f"✅ Successfully processed schedule id={r.get('id')} for {target_date}")
                except Exception as ex:
                    logging.error(f"❌ Error processing schedule id={r.get('id')} for {target_date} -> rolled back")
                    logging.error(str(ex), exc_info=True)
                    conn.rollback()
    return inserted_attempts

def generate_events_for_three_days(start_date: Optional[date] = None) -> int:
    """Generate events for today + next 2 days"""
